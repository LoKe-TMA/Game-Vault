document.addEventListener("DOMContentLoaded", () => {
    ...
    if(dailyAdBtn){
        const AdController = window.Adsgram.init({ blockId: "int-13300" }); // သင့် AdsGram blockId ဖြည့်ပါ
        dailyAdBtn.addEventListener("click", () => {
            AdController.show()
                .then(() => {
                    // Reward user
                    alert('✅ You earned coins & spins!');
                    fetch('https://gamevault-backend-nf5g.onrender.com/api/tasks/complete', {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ telegramId: user.id, taskId: DAILY_TASK_ID })
